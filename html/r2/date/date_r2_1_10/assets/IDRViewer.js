...
function makeNavBar(pageCount, curPage, w, h, t, heights) {
    ...
    if (!fullscreenEnabled || isMobile) {
        var rm = d('btnFullscreen'); rm.parentNode.removeChild(rm);
    }
...
function loadThumbnailFrames(heights){
    ...
    for(var page = 1; page <= pgCount; page++){
        ...
        div.innerHTML='<img src="assets/loading.gif"/>';
...
function goToPage(pg) {
    ...
    if (pg != curPg) {
        ...
        if (type == 0) {
            ...
            window.open(link + ".htm" + params, "_self");
        } else if (type == 1) {
...
function fullscreenUpdate() {
    if (!document.fullscreenElement && !document.msFullscreenElement && !document.mozFullScreenElement && !document.webkitFullscreenElement) {
        d('btnFullscreen').children[0].src = "assets/pgFullScreen.png";
    } else {
        d('btnFullscreen').children[0].src = "assets/pgFullScreenClose.png";
    }
}
...
function addToolBarHyperlink(img, link) {
